...
class recipientgroupmanager
{
    ...
    {
        if (is_file(WORKSPACE . "/email-newsletters/recipient_group.".str_replace('-', '_', $handle).".php") || $new == true) {
            ...
            if (is_array($extensions) && !empty($extensions)) {
                foreach ($extensions as $e) {
                    if (is_file(EXTENSIONS . "/$e/email-newsletters/recipient_group.".str_replace('-', '_', $handle).".php")) {
    ...
    public static function save($handle = null, $fields)
    {
        ...
        if ($handle == Lang::createHandle($fields['name'], 255, '-') || (($handle == null) && (self::__getClassPath(Lang::createHandle($fields['name'], 255, '-')) == false))) {
            ...
            throw new Exception('Recipientsource ' . $fields['name'] . ' already exists. Please choose a different name.');
        }
    }
